...
import pickle
import random
from models_second import Mixer, Discriminator, HypyerNet, QNN
import h5py
np.random.seed(1234)
...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, default='FF_Qualtile_Loss')
...
def main():
    ...
    for ed in [100]:
        for beta in [1.0]:
            ...
            results_folder_name = os.path.join(config['path_dir'], 'results_quantile')
            try:
                os.mkdir(results_folder_name)
            except OSError:
                pass
            folder_name = results_folder_name + '/' + '{}_{}_{}_{}_{}'.format(config['model_name'], config['ed_multiple'], config['embedding_dim'], config['beta'], config['rnd'])
            try:
            ...
            if not os.path.exists(check_name):
                ...
                model = QNN(21 + config['embedding_dim'] * 2, len(quantiles))
                ...
                class QuantileLoss(nn.Module):
                    ...
                    def __init__(self, quantiles, weights):
                        super().__init__()
                    ...
                    def forward(self, target, preds):
                        ...
                        for i, q in enumerate(self.quantiles):
                            errors = target * self.w - preds[:, :, i, :] * self.w
                            losses.append(torch.max((q - 1) * errors, q * errors).unsqueeze(1))
                        loss = torch.mean(torch.sum(torch.cat(losses, dim=1), dim=1))
                        return loss
                loss_q = QuantileLoss(quantiles, weights)
                ...
                def train(epoch, dataloader):
                    ...
                    for bidx, samples in enumerate(dataloader):
                        ...
                        if bad_included == 'True':
                            ...
                            all_mean = torch.mul(all_output, mask.unsqueeze(2).repeat(1, 1, len(quantiles), 1))
                            all_target = torch.mul(target, mask)
                        else:
                            all_mean = all_output
                            all_target = target
                        loss = loss_q(all_target, all_mean)
                        loss.backward()
                        optimizer.step()
                ...
                def pred(data):
                    ...
                    loss_mean = output[:, :, :, 0:1]
                    loss_cum_mean = torch.cumsum(loss_mean, dim=1)
                    all_output = torch.cat((output, loss_cum_mean), dim=-1).detach().cpu().numpy()
                    lower_bound = all_output[:, :, 0, :]
                    predictive_mean = all_output[:, :, 1, :]
                    upper_bound = all_output[:, :, 2, :]
