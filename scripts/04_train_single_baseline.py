...
import pickle
import random
from models_second import Mixer, Discriminator, HypyerNet, QNN, SNN
import h5py
np.random.seed(1234)
...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, default='FF_Single_Loss')
    ...
    parser.add_argument('--embedding_dim', type=int, default=0, help='embedding dimension for the FFE')
    parser.add_argument('--beta', type=float, default=50.0, help='beta')
    parser.add_argument('--ed_multiple', type=int, default=5, help='multiple of embedding dim')
    args = parser.parse_args()
    config = OrderedDict([('path_dir', args.path_dir), ('model_name', args.model_name), ('dataset', args.dataset), ('epochs', args.epochs), ('n_models', args.n_models), ('batch_size', args.batch_size), ('lr', args.lr), ('lr_decay', args.lr_decay), ('rnd', args.rnd), ('mode', args.mode), ('embedding_dim', args.embedding_dim), ('beta', args.beta), ('ed_multiple', args.ed_multiple)])
    return config
...
def main():
    ...
    rnds = np.random.randint(0, 2 ** 20, 50)
    rnd = rnds[config['rnd']]
    for ed in [100]:
        for beta in [1.0]:
            config['beta'] = beta
            config['embedding_dim'] = ed
            config['ed_multiple'] = 1
            ...
            results_folder_name = os.path.join(config['path_dir'], 'results_single')
            try:
                os.mkdir(results_folder_name)
            except OSError:
                pass
            folder_name = results_folder_name + '/' + '{}_{}_{}_{}_{}'.format(config['model_name'], config['ed_multiple'], config['embedding_dim'], config['beta'], rnd)
            try:
                os.mkdir(folder_name)
            except OSError:
                pass
            check_name = folder_name + '/model_final.pth'
            logvar_min = -10.0
            logvar_max = 5.0
            ...
            if not os.path.exists(check_name):
                ...
                def train(epoch, dataloader):
                    ...
                    for bidx, samples in enumerate(dataloader):
                        ...
                        mean_, logvar_ = model(inputs)
                        loss_mean = mean_[:, :, 0:1]
                        loss_cum_mean = torch.cumsum(loss_mean, dim=1)
                        all_output = torch.cat((mean_, loss_cum_mean), dim=-1)
                        loss_log_var = logvar_[:, :, 0:1]
                        loss_log_var = loss_log_var.clamp(min=logvar_min, max=logvar_max)
                        loss_cum_log_var = torch.cumsum(loss_log_var, dim=1)
                        all_uq = torch.cat((logvar_, loss_cum_log_var), dim=-1)
                        optimizer.zero_grad()
                        if bad_included == 'True':
                            mask = Variable(samples[2]).to(device)
                            all_mean = torch.mul(all_output, mask)
                            all_logvar = torch.mul(all_uq, mask)
                            all_target = torch.mul(target, mask)
                        else:
                            all_mean = all_output
                            all_target = target
                        loss = nll_loss(all_mean, all_logvar, all_target, weights)
                        loss.backward()
                        optimizer.step()
                ...
                def test(epoch):
                    ...
                    for bidx, samples in enumerate(val_dataloader):
                        ...
                        loss_cum_log_var = torch.cumsum(loss_log_var, dim=1)
                        all_uq = torch.cat((logvar_, loss_cum_log_var), dim=-1)
                        loss = nll_loss(all_output, all_uq, target, weights)
                        epoch_loss += loss.detach().cpu().numpy()
                    epoch_loss /= len(val_dataloader)
                    print('Test Epoch: {}/{} Loss: {:.4f}'.format(epoch, config['epochs'], epoch_loss))
                    mean_ = mean_.detach().cpu().numpy()
                    return (mean_, epoch_loss)
                ...
                def pred(data):
                    ...
                    loss_cum_log_var = torch.cumsum(loss_log_var, dim=1)
                    all_uq = torch.cat((logvar_, loss_cum_log_var), dim=-1)
                    std_ = torch.exp(0.5 * all_uq)
                    lower_bound = all_output - 1.96 * std_
                    upper_bound = all_output + 1.96 * std_
                    predictive_mean = all_output
                    return (mean_, predictive_mean.detach().cpu().numpy(), lower_bound.detach().cpu().numpy(), upper_bound.detach().cpu().numpy())
                ...
                def run():
                    ...
                    for epoch in range(config['epochs']):
                        ...
                        vlosses.append(vepoch_loss)
                        if vepoch_loss < vloss_min:
                            vloss_min = vepoch_loss
                            torch.save(model.state_dict(), os.path.join(folder_name, 'model_best.pth'))
                            draw('best')
                    torch.save(model.state_dict(), os.path.join(folder_name, 'model_final.pth'))
                    draw('final')
