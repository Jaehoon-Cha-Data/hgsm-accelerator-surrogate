...
def main():
    ...
    results_folder_name = os.path.join(config['path_dir'], 'results_second_bad_true_uq_1000_epoch')
    try:
    ...
    if not os.path.exists(check_name):
        ...
        hyper_pretrained_dict = torch.load(results_folder_name + '/hyper_best.pth')
        hyper_state_dict = hyper.state_dict()
        for key in hyper_state_dict.keys():
            if key in hyper_pretrained_dict:
                if hyper_state_dict[key].shape == hyper_pretrained_dict[key].shape:
                    hyper_state_dict[key] = hyper_pretrained_dict[key]
                    print(f'Loaded key {key} from pretrained model.')
            elif 'layerd1q' in key:
                mirror_key = key.replace('layerd1q', 'layerd1')
                if mirror_key in hyper_pretrained_dict:
                    if hyper_state_dict[key].shape == hyper_pretrained_dict[mirror_key].shape:
                        hyper_state_dict[key] = hyper_pretrained_dict[mirror_key]
                        print(f'Initialized {key} from pretrained {mirror_key}.')
                    else:
                        print(f'Shape mismatch: {key} and {mirror_key}')
                else:
                    print(f'No matching key {mirror_key} found in pretrained model for {key}')
        hyper.load_state_dict(hyper_state_dict)
        diser_pretrained_dict = torch.load(results_folder_name + '/diser_best.pth')
        diser.load_state_dict(diser_pretrained_dict)
        mixer_pretrained_dict = torch.load(results_folder_name + '/mixer_best.pth')
        mixer_state_dict = mixer.state_dict()
        for key in mixer_state_dict.keys():
            if key in mixer_pretrained_dict:
                if mixer_state_dict[key].shape == mixer_pretrained_dict[key].shape:
                    mixer_state_dict[key] = mixer_pretrained_dict[key]
                    print(f'Loading key {key} from pretrained model.')
        ...
        def d_loss(pred_fake, pred_real):
            target_real = torch.full((config['n_models'], 1), 1.0, device=device, dtype=torch.float32)
            target_fake = torch.full((model_params['mixer_args']['n_layers'], config['n_models'], 1), 0.0, device=device, dtype=torch.float32)
            real_loss_d = torch.nn.MSELoss()(pred_real, target_real)
            fake_loss_d = torch.nn.MSELoss()(pred_fake, target_fake)
        ...
        def train(epoch, dataloader):
            ...
            for bidx, samples in enumerate(dataloader):
                ...
                target_g = torch.full((model_params['mixer_args']['n_layers'], config['n_models'], 1), 1.0, device=device, dtype=torch.float32)
                loss_g = torch.nn.MSELoss()(pred_codes, target_g)
                if bad_included == 'True':
