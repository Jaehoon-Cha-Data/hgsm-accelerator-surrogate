...
class HypyerNet(nn.Module):
    def __init__(self, params):
        ...
        self.layerd1q = Generator(types.SimpleNamespace(**params['d1q_args']))
    ...
    def get_weights(self, z):
        ...
        wd1q, bd1q = self.layerd1q(z[4])
        return w0, b0, w1, b1, wd0, bd0, wd1, bd1, wd1q, bd1q
    ...
    def forward(self, z, x):
        ...
        w0, b0, w1, b1, wd0, bd0, wd1, bd1, wd1q, bd1q = self.get_weights(z)
        ...
        y_d1q = self.dense_block(y_d0, wd1q, bd1q, act = nn.Identity)

        y_d1[:, :, :, 0] = nn.ReLU()(y_d1[:, :, :, 0])
        y = y_d1.permute(2, 0, 1, 3)
        yq = y_d1q.permute(2, 0, 1, 3)
...
class QNN(nn.Module):
    def __init__(self, input_dim, n_quantiles):
        ...
        self.fc4 = nn.Linear(128, self.out_features*n_quantiles)
    ...
    def forward(self, x):
        ...
        if self.n_quantiles > 1:
            x = x.view(-1, 200, self.n_quantiles, self.out_features)
        x[:, :, :, 0] = nn.ReLU()(x[:, :, :, 0])
        return x
...
class SNN(nn.Module):
    def __init__(self, input_dim):
        ...
        self.fc2 = nn.Linear(128, 256)
        self.fc3 = nn.Linear(256, 128)
        self.fc4m = nn.Linear(128, self.out_features)
        self.fc4q = nn.Linear(128, self.out_features)
